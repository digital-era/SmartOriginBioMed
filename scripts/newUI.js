...
function selectCategory(category) {
    ...
    renderCategoryLayout(category);
      ...
      // 【关键】使用合并后的数据
    const candidates = getFilteredCandidates(category);  // ← 必须用这个，不是 allData[category]
...
function renderCategoryLayout(category) {
  ...
  const navTooltip = translations[lang]?.tooltipInterstellarNav || (lang === 'en' ? 'Interstellar Navigator' : '星际领航');
  const intraNavTooltip = translations[lang]?.intraNavTooltip || (lang === 'en' ? 'Intra-stellar Navigator' : '星内领航');
    ...
    <div class="layout-right">
      <div class="card-stage">
        <div class="search-and-random">
          ...
          <!-- 【新增】星内领航按钮 -->
          <button 
            class="navigator-trigger-btn intra-navigator-trigger-btn" 
            id="btn-intra-navigator" 
            title="${intraNavTooltip}" 
            aria-label="${intraNavTooltip}"
            style="
              width: 32px; 
              height: 32px; 
              padding: 0; 
  ...
  // 【新增】星内领航按钮事件绑定
  const intraNavBtn = document.getElementById('btn-intra-navigator');
  if (intraNavBtn) {
      intraNavBtn.addEventListener('click', () => {
          activateIntraStellarNavigator();
      });
  }
...
function updateSingleCard(leader) {
    ...
    `;
      ...
      if (category && checkUserPermission()) {
          renderCustomNorthStarControls(card, leader, category);
      }
}
...
function getFilteredCandidates(category) {
  ...
  if (activeSub) {
    ...
    candidates = candidates.filter(m => {      
      // 【修复】同样使用 getFieldValue
      //const fieldText = m.field ? (m.field[lang] || m.field['zh-CN'] || '').toLowerCase() : '';
      const fieldText = getFieldValue(m.field, lang).toLowerCase();      
  ...
  if (searchInput && searchInput.value.trim()) {
    ...
    candidates = candidates.filter(m => {
        // 【修复】使用 getFieldValue 统一处理字符串和多语言对象
        const nameText     = getFieldValue(m.name, lang);
        const contribText  = getFieldValue(m.contribution, lang);
        const fieldText    = getFieldValue(m.field, lang);
        const remarksText  = getFieldValue(m.remarks, lang);
  ...
  console.log(`[Filter] category=${category}, activeSub=${activeSub}, search="${searchInput?.value}", builtIn=${builtIn.length}, custom=${custom.length}, matched=${candidates.length}`);
  return candidates;
}
